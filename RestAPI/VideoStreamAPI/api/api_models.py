...
def get_star_model(api, include_relationship=True):
    if include_relationship:
        video_model = get_video_meta_response_model(
            api=api, include_relationship=False)
...
def get_director_model(api,  include_relationship=True):
    if include_relationship:
        videos_model = get_video_meta_response_model(
            api=api, include_relationship=False)
...
def get_genre_model(api,  include_relationship=True):
    if include_relationship:
        videos = get_video_meta_response_model(
            api=api, include_relationship=False)
...
def get_video_request_parser(api):
    ...
    request_parser.add_argument("sort_by", type=str, choices=[
                                "genre", "star", "director", "title", "duration_seconds", "upload_date", "language"])
    request_parser.add_argument("order", type=str, choices=[
                                "asc", "desc"], default="asc")

    # e.g., ?min_duration=180
    request_parser.add_argument("min_duration", type=int)
